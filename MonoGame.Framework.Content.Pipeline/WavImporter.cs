    ...
    public class WavImporter : ContentImporter<AudioContent>
    {
        ...
        public override AudioContent Import(string filename, ContentImporterContext context)
        {
            ...
            if (content.Format.SampleRate < 8000 || content.Format.SampleRate > 48000)
                throw new InvalidContentException($"Audio file {Path.GetFileName(filename)} contains audio data with unsupported sample rate of {content.Format.SampleRate}KHz. Supported sample rates are from 8KHz up to 48KHz.");
