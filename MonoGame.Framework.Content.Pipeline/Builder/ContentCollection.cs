...
/// <summary>
/// Collection of data on how the content should be handled.
/// </summary>
public class ContentCollection : IContentCollection
{
    ...
    public void IncludeCopy(string inputPath, string? outputPath)
        => _imputFiles[inputPath] = new(false, null, null, string.IsNullOrWhiteSpace(outputPath) ? ContentInfo.GetDefaultOutputPath : s => outputPath);
    ...
    public void Include(string inputPath, IContentImporter? contentImporter = null, IContentProcessor? contentProcessor = null)
        => _imputFiles[inputPath] = new(true, contentImporter, contentProcessor);
    ...
    {
        _imputFiles[inputPath] = new(true, contentImporter, contentProcessor, (s) => outputPath);
    }
        ...
        where T : ContentRule, new()
    {
        ...
        _rules.Add((rule, new(false, null, null, outputPath)));
    }
        ...
        where T : ContentRule, new()
    {
        var rule = new T { Pattern = includePattern };
        RemoveFilesByRule(rule);
        _rules.Add((rule, new(true, contentImporter, contentProcessor)));
    }
        ...
        where T : ContentRule, new()
    {
        var rule = new T { Pattern = includePattern };
        RemoveFilesByRule(rule);
        _rules.Add((rule, new(true, contentImporter, contentProcessor, outputPath)));
    }
