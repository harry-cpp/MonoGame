...
namespace MonoGame.Content.Builder.Editor
{
    ...
    public class ProcessorConverter : TypeConverter
    {
        ...
        {
            if (context.Instance is Array array)
            {
                foreach (var obj in array)
                {
                    if (obj is ContentItem item && item.BuildAction == BuildAction.Copy)
                    {
                        return false;
                    }
                }
            ...
            {
                if (context.Instance is ContentItem contentItem && contentItem.BuildAction == BuildAction.Copy)
                {
                    return false;
                }
            }
        ...
        public override object ConvertTo(ITypeDescriptorContext context, CultureInfo culture, object value, Type destinationType)
        {
            if (destinationType == typeof(string))
            {
                ...
                if (processor == PipelineTypes.MissingProcessor)
                {
                    ...
                    return string.Format("[missing] {0}", contentItem == null ? "null" : contentItem.ProcessorName);
                }
        ...
        public override PropertyDescriptorCollection GetProperties(ITypeDescriptorContext context, object value, Attribute[] attributes)
        {
            ...
            if (context.Instance is object[] array)
            {
                ...
                {
                    var prop = new OpaqueDataDictionaryElementPropertyDescriptor(p.Name, p.Type);
            ...
            else
            {
                ...
                if (value == PipelineTypes.MissingProcessor)
                {
                    ...
                    {
                        var desc = new OpaqueDataDictionaryElementPropertyDescriptor(p.Key, p.Value.GetType(), contentItem.ProcessorParams);
                        props.Add(desc);
                    }
                }
                else
                {
                    ...
                    foreach (var p in processor.Properties)
                    {
                        var desc = new OpaqueDataDictionaryElementPropertyDescriptor(p.Name, p.Type, contentItem.ProcessorParams);
                        props.Add(desc);
                    }
                }
            }
        ...
        public override bool GetPropertiesSupported(ITypeDescriptorContext context)
        {
            ...
            if (context.Instance is Array)
            {
                ...
                if (first == null || !first.Processor.Properties.Any())
                    return false;
