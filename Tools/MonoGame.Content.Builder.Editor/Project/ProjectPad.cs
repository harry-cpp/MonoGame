...
namespace MonoGame.Content.Builder.Editor.Project
{
    public partial class ProjectPad : Pad
    {
        ...
        public ProjectPad()
        {
            ...
            foreach (var t in typeof(ProjectPad).Assembly.GetTypes())
            {
                if (!t.IsAbstract && typeof(ProjectPadCommand).IsAssignableFrom(t))
                {
                    ...
                    cmd.OnIsActive(this, new List<IProjectItem>(), new List<TreeGridItem>());
            ...
            foreach (var cmd in _commands)
            {
                if (cmd.IsInMenu)
                {
                    AddMenuItem("Edit/" + cmd.Category + "/" + cmd.GetName(null), cmd);
                }
            }
        ...
        private void TreeView_SelectionChanged(object sender, EventArgs e)
        {
            ...
            PipelineController.Instance.LoadProperties(items);
            ...
            {
                if (!item.OnIsActive(this, items, treeItems))
                    continue;
